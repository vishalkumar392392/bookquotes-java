...
public class BookServiceImpl implements BookService{
	...
	public List<BookModel> getAllBooks() {
		List<BookModel> books = bookRepository.findAll().stream().map(bookMapper::getModel).collect(Collectors.toList());
		List<Quote> quotes = quoteRepository.findAll();
		
		for(BookModel bookModel:books) {
			bookModel.setQuotes(quotes.stream().filter(x->x.getBookId()==bookModel.getId()).map(Quote::getQuote).collect(Collectors.toList()));
		}
		return books;
	}
	@Override
